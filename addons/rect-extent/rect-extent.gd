...
func forward_canvas_draw_over_viewport(overlay: Control) -> void:
	...
	var pos = rect_extents.global_position
	...
	var editor_anchors : = {
		...
		ANCHORS.TOP_RIGHT: pos + Vector2(half_size.x, -half_size.y) + offset,
		ANCHORS.BOTTOM_LEFT: pos + Vector2(-half_size.x, half_size.y) + offset,
